...
profiles = {
    'good': {
        ...
        'negative': lambda:
            expon.rvs(loc=0, scale=np.random.choice([.5, 1])).astype(int),
        'chance': .5
    },
    'normal': {
        'positive': lambda: norm.rvs(loc=norm.rvs(3), scale=1).astype(int),
        'negative': lambda:
            norm.rvs(loc=2, scale=np.random.choice([.5, 1, 2, 3])).astype(int),
        'chance': .15
    },
    'poor': {
    ...
    'chaotic_good': {
        ...
        'negative': lambda:
            np.random.choice([0, np.random.choice([50, 200])], p=[.98, .02]),
        'chance': .2
    },
    'chotic_bad': {
        'positive': lambda: expon.rvs(loc=0, scale=5).astype(int),
...
notes = pd.DataFrame(_, columns=['employee_id', 'employee_name',
                                 'note']).assign(
            event_date=np.random.choice(df.event_date,
